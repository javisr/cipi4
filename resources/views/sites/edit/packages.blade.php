    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-8 py-8">
                ...
                @if (session('packagesUpdated'))
                <div class="rounded-md bg-green-50 p-4 mb-2" id="packagesUpdated">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <h3 class="text-sm text-green-800 font-bold">{{ __('Site auth.json file for packages has been updated!') }}</h3>
                        </div>
                    </div>
                </div>
                <script>
                    setTimeout(() => {
                        $("#packagesUpdated").hide(500);
                    }, 3500);
                </script>
                @endif
                ...
                <div class="pb-4 px-4 sm:px-0">
                    ...
                    <form class="space-y-8" method="post" action="/sites/{{ $site }}/edit/packages">
                        <input type="hidden" name="packages" id="packagesField">
                        @csrf

                        <div class="space-y-8  sm:space-y-5">

                            <div class="sm:grid sm:grid-cols-3 sm:gap-4">
                                <label for="packages" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"> {{ __('Site auth.json for packages') }} </label>
                                <div class="mt-1 sm:mt-0 sm:col-span-2">
                                    <div class="max-w-lg flex rounded-md shadow-sm">
                                        <div id="packages" name="packages" class="rounded-md" style="width: 100%; height: 220px;">{{ $packages }}</div>
                                        <script>
                                            var editor = ace.edit("packages");
                                            editor.setTheme("ace/theme/monokai");
                                            editor.getSession().setMode("ace/mode/json");
                                            editor.getSession().on('change', function() {
                                                $('#packagesField').val(editor.getSession().getValue());
                                            });
                                        </script>
                                    </div>
                                </div>
